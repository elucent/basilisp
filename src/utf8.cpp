...
ustring::ustring(const const_slice<uchar>& range): ustring() {
    for (uchar c : range) operator+=(c);
}
...
const uchar& ustring::operator[](u32 i) const {
    return data[i];
}
...
const_slice<uchar> ustring::operator[](pair<u32, u32> range) const {
    return { range.second - range.first, data + range.first };
}

slice<uchar> ustring::operator[](pair<u32, u32> range) {
    return { range.second - range.first, data + range.first };
}
...
void write(stream& io, const const_slice<uchar>& str) {
    for (uchar c : str) write(io, c);
}

void write(stream& io, const slice<uchar>& str) {
    for (uchar c : str) write(io, c);
}
